---
notion_page_id: "2db309d2-5a8c-805e-9698-f78d6c5438e3"
...
- `{purpose}` is optional for multiple instances of the same resource type.
- `{nanoid}` is resource-scoped and deterministic within deployment.
...
| Length (Storage Account) | 8 characters |
...
Publisher operates two Key Vaults.

**Private Key Vault** follows standard settings.
...
- Automation Account is deployed empty.
- Runbooks are packaged as OCI artifacts in Publisher ACR.
...
---
## 13. AI Services
Azure AI Search and Azure AI Foundry are separate services.
...
All resources must have diagnostic settings configured to emit logs and metrics to Log Analytics Workspace.

| Setting | Value |
| --- | --- |
| Destination | Log Analytics Workspace |
| Metrics | AllMetrics |
| Naming Convention | `diag-{resourceName}` |

### 19.2 Log Categories by Resource Type

| Resource Type | Log Categories | LAW Table |
| --- | --- | --- |
| Container Apps | ContainerAppConsoleLogs, ContainerAppSystemLogs | ContainerAppConsoleLogs, ContainerAppSystemLogs |
| App Service | AppServiceHTTPLogs, AppServiceConsoleLogs, AppServiceAppLogs | AppServiceHTTPLogs, AppServiceConsoleLogs |
| Function App | FunctionAppLogs | FunctionAppLogs |
| Key Vault | AuditEvent | AzureDiagnostics |
| Storage Account | StorageRead, StorageWrite, StorageDelete | StorageBlobLogs |
| PostgreSQL | PostgreSQLLogs, QueryStoreRuntimeStatistics | AzureDiagnostics |
| ACR | ContainerRegistryRepositoryEvents, ContainerRegistryLoginEvents | ContainerRegistryRepositoryEvents, ContainerRegistryLoginEvents |
| AKS | kube-apiserver, kube-controller-manager, kube-scheduler | AzureDiagnostics |
| Application Gateway | ApplicationGatewayAccessLog, ApplicationGatewayPerformanceLog, ApplicationGatewayFirewallLog | ApplicationGatewayAccessLog, ApplicationGatewayFirewallLog |
| Automation Account | JobLogs, JobStreams | AzureDiagnostics |
...
---
# Open Questions
1. **Log Retention Tiering:** Should we implement tiered retention for cost optimization (hot vs cold tiers)? Owner: Platform Team.
2. **Log Ingestion Cost Controls:** Consider safeguards such as daily caps, per-service quotas, or alerting-only with anomaly detection. Owner: Platform Team.
...
| --- | --- | --- | --- |
| 1.0 | Initial | - | Original RFC-71 |
| 1.1 | 2026-01-16 | Claude | Added Container Apps to Section 19.2; added LAW Table column; added diagnostic settings naming convention `diag-{resourceName}`; added Open Questions for log retention tiering and cost controls |
