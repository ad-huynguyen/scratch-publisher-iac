---
notion_page_id: "2d9309d2-5a8c-8046-b2df-ff34ea740661"
...
---
# Summary
Defines the CI/CD standards to be followed by the vibedata engineering team and covers repository structure, branching strategy, and release pipeline for VibeData. GitHub is the single platform for source control, CI/CD, and artifact storage. Uses GitHub Flow for all repositories with two distinct deployment flows: Publisher (GitHub Flow → ACR → Terraform) and Managed App (GitHub Flow → ACR → Manifest Repo → GitHub Artifact Store → Production). Does not cover CI/CD for customer environments for their fabric artifacts (RFC-40).
...
| Managed App | `platform-services` | Core platform services (Health, Registry, Update) |
...
| Managed App | `marketplace-iac` | Bicep templates for Marketplace deployment |
...
| `created` | ISO timestamp | GitHub Actions |
